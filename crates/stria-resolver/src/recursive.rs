...
use stria_dnssec::{DefaultTrustAnchorStore, DnskeyFetcher, DnssecValidator, ValidationResult};
use stria_proto::{
...
impl RootHints {
    ...
    pub fn get_servers(&self) -> Vec<SocketAddr> {
        ...
        entries
            .into_iter()
            .flat_map(|e| e.addresses.iter().map(|addr| SocketAddr::new(*addr, 53)))
            .collect()
    }
...
// ============================================================================
// DNSKEY Fetcher for Resolver
// ============================================================================

/// DNSKEY fetcher implementation for the recursive resolver.
///
/// This fetcher can retrieve DNSKEY and DS records by performing DNS queries
/// to the appropriate authoritative servers. It's used by the DNSSEC validator
/// when DNSKEY records are needed but not present in the response being validated.
#[derive(Clone)]
pub struct ResolverKeyFetcher {
    /// Root server addresses for querying root zone.
    root_servers: Vec<SocketAddr>,
    /// Query timeout duration.
    timeout: Duration,
    /// NS cache for resolving non-root zones (shared with resolver).
    ns_cache: Arc<DashMap<Name, NsCacheEntry>>,
}

impl ResolverKeyFetcher {
    /// Creates a new key fetcher with the given root hints.
    pub fn new(root_hints: &RootHints, ns_cache: Arc<DashMap<Name, NsCacheEntry>>) -> Self {
        Self {
            root_servers: root_hints.get_servers(),
            timeout: Duration::from_secs(5),
            ns_cache,
        }
    }

    /// Performs a simple UDP query to a server.
    async fn query_server(&self, addr: SocketAddr, query: &Message) -> Option<Message> {
        ...
        socket.send_to(&wire, addr).await.ok()?;

        let mut buf = vec![0u8; 4096];
        let result = timeout(self.timeout, socket.recv_from(&mut buf)).await;

        match result {
            Ok(Ok((len, _))) => Message::parse(&buf[..len]).ok(),
            _ => None,
        }
    }

    /// Queries multiple servers and returns the first successful response.
    async fn query_servers(&self, servers: &[SocketAddr], query: &Message) -> Option<Message> {
        // Try servers in parallel, return first success
        let futures: Vec<_> = servers
            .iter()
            .take(3) // Query at most 3 servers
            .map(|addr| self.query_server(*addr, query))
            .collect();

        for future in futures {
            if let Some(response) = future.await {
                // Check for valid response (no SERVFAIL, has answers)
                if !response.is_servfail() && !response.answers().is_empty() {
    ...
    /// Gets servers for a zone from the NS cache.
    fn get_servers_for_zone(&self, zone: &Name) -> Option<Vec<SocketAddr>> {
        ...
        if let Some(entry) = self.ns_cache.get(zone) {
            if entry.expires_at > Instant::now() {
                let addrs: Vec<SocketAddr> = entry
                    .nameservers
                    .iter()
                    .flat_map(|ns| ns.addresses.iter().map(|ip| SocketAddr::new(*ip, 53)))
                    .collect();
                if !addrs.is_empty() {
                    return Some(addrs);
                }
...
impl stria_dnssec::DnskeyFetcher for ResolverKeyFetcher {
    async fn fetch_dnskey(&self, zone: &Name) -> Option<Message> {
        // Build DNSKEY query
        let question = Question::new(zone.clone(), RecordType::DNSKEY, RecordClass::IN);
        let mut query = Message::query_dnssec(question);
        query.header_mut().set_recursion_desired(false); // We want authoritative answer

        trace!("Fetching DNSKEY for zone: {}", zone);

        // Determine which servers to query
        let servers = if zone.is_root() {
            self.root_servers.clone()
        } else {
            // Try to find NS for this zone
            self.get_servers_for_zone(zone)
                .unwrap_or_else(|| self.root_servers.clone())
        };

        self.query_servers(&servers, &query).await
    }

    async fn fetch_ds(&self, zone: &Name) -> Option<Message> {
        // Build DS query
        let question = Question::new(zone.clone(), RecordType::DS, RecordClass::IN);
        let mut query = Message::query_dnssec(question);
        query.header_mut().set_recursion_desired(false);

        trace!("Fetching DS for zone: {}", zone);

        // DS records are in the parent zone, so we query the parent's servers
        let parent = zone.parent()?;
        let servers = if parent.is_root() {
            self.root_servers.clone()
        } else {
            self.get_servers_for_zone(&parent)
                .unwrap_or_else(|| self.root_servers.clone())
        };
...
pub struct RecursiveResolver {
    ...
    dnssec_validator: Option<Arc<DnssecValidator<DefaultTrustAnchorStore, ResolverKeyFetcher>>>,
    ...
    /// Set of NS names currently being resolved (to prevent infinite recursion).
    resolving_ns: Arc<DashSet<Name>>,

    /// Zones proven to be insecure (unsigned) delegations.
    /// These zones have NSEC/NSEC3 records proving no DS exists at the parent.
    insecure_zones: Arc<DashSet<Name>>,
}
...
impl RecursiveResolver {
    ...
    pub fn with_config(config: RecursiveConfig, cache: Arc<DnsCache>) -> Self {
        let root_hints = RootHints::default();
        let ns_cache = Arc::new(DashMap::new());

        let dnssec_validator = if config.enable_dnssec {
            // Create a DNSKEY fetcher that can query for keys
            let fetcher = ResolverKeyFetcher::new(&root_hints, Arc::clone(&ns_cache));
            Some(Arc::new(DnssecValidator::with_fetcher(
                DefaultTrustAnchorStore::default(),
                fetcher,
            )))
        } else {
            None
        ...
        Self {
            ...
            resolving_ns: Arc::new(DashSet::new()),
            insecure_zones: Arc::new(DashSet::new()),
        }
    }
    ...
    pub fn with_dnssec_validator(
        config: RecursiveConfig,
        cache: Arc<DnsCache>,
        validator: Arc<DnssecValidator<DefaultTrustAnchorStore, ResolverKeyFetcher>>,
    ) -> Self {
        Self {
            config,
            cache,
    ...
    /// Checks if a name is within an insecure (unsigned) zone.
    ///
    /// Returns true if the name equals or is a subdomain of any zone
    /// that has been proven to have no DS record (insecure delegation).
    fn is_in_insecure_zone(&self, name: &Name) -> bool {
        for zone in self.insecure_zones.iter() {
            if name == zone.key() || name.is_subdomain_of(zone.key()) {
                return true;
            }
        }
        false
    ...
    /// Marks a zone as insecure (no DS record exists).
    fn mark_zone_insecure(&self, zone: &Name) {
        if !self.insecure_zones.contains(zone) {
            debug!(zone = %zone, "Marking zone as insecure (no DS record)");
            self.insecure_zones.insert(zone.clone());
        }
    }
    ...
    async fn resolve_iterative(&self, state: &mut ResolutionState) -> Result<Message> {
        loop {
            ...
            let response = self
                .query_nameservers(&ns_addrs, &query_question, state)
                .await?;
    ...
    async fn query_nameservers(
        ...
        let futures: Vec<_> = servers_to_try
            ...
            .map(|addr| {
                ...
                async move {
                    let result = self
                        .query_single_server(addr, &query, timeout_duration)
                        .await;
                    (addr, result)
                }
            })
            ...
            match result {
                Ok(response) => {
                    ...
                    if state.dnssec_requested {
                        // Skip DNSSEC validation for names in insecure zones
                        // (zones proven to have no DS record at the parent)
                        if self.is_in_insecure_zone(&state.current_qname) {
                            trace!(
                                qname = %state.current_qname,
                                "Skipping DNSSEC validation - zone is insecure"
                            );
                        } else if let Some(validator) = &self.dnssec_validator {
                            self.metrics
                                .dnssec_validations
                    ...
                    match &best_response {
                        ...
                        Some((_, prev))
                            if !prev.is_authoritative() && response.is_authoritative() =>
                        {
                            best_response = Some((addr, response));
                        }
                        _ => {}
        ...
        match best_response {
            ...
            None => Err(last_error.unwrap_or(ResolverError::AllUpstreamsFailed)),
        }
    }
    ...
    async fn query_single_server(
        ...
        // Try UDP first
        let udp_result = timeout(timeout_duration, self.query_udp(addr, query)).await;
    ...
    async fn process_response(
        ...
        if response.is_nxdomain() {
            ...
            let mut final_response =
                Message::response_from(&Message::query(state.original_query.clone()));
                ...
                if record.record_type() == Some(RecordType::DNAME) {
                    if let RData::DNAME(dname) = record.rdata() {
                        ...
                        if let Some(new_name) =
                            self.apply_dname(&state.current_qname, record.name(), dname.target())
                        {
    ...
    fn build_final_response(&self, response: &Message, state: &ResolutionState) -> ResponseAction {
        let mut final_response =
            Message::response_from(&Message::query(state.original_query.clone()));
    ...
    async fn process_referral(
        ...
        let zone =
            zone.ok_or_else(|| ResolverError::Protocol("Referral without NS records".into()))?;
        ...
        for ns_name in &ns_names {
            ...
            for record in response.additional() {
                ...
                // Bailiwick check for glue: the glue must be in the bailiwick of the
                // server that provided it (the current_zone), OR in the delegated zone.
                // This allows root servers to provide glue for .net NS when referring to .com
                // because root is authoritative for both .com and .net.
                let glue_valid = self.is_in_bailiwick(ns_name, &state.current_zone)
                    || self.is_in_bailiwick(ns_name, &zone);
        ...
        if nameservers.is_empty() {
            return Err(ResolverError::Protocol(
                "No usable nameservers in referral".into(),
            ));
        }
        ...
        // Check if this is a secure or insecure delegation
        let has_ds = response
            .authority()
            .iter()
            .any(|r| r.record_type() == Some(RecordType::DS));

        // If no DS record exists, this is an insecure delegation
        // Mark it so we skip DNSSEC validation for queries to this zone
        if !has_ds {
            self.mark_zone_insecure(&zone);
        }
    ...
    fn resolve_ns_addresses<'a>(&'a self, name: &'a Name) -> BoxFuture<'a, Vec<IpAddr>> {
        async move {
            // Check if we're already resolving this NS name (prevent infinite recursion)
            if self.resolving_ns.contains(name) {
                debug!(ns = %name, "Already resolving this NS name, skipping to prevent recursion");
                return Vec::new();
            }

            // Mark this NS name as being resolved
            self.resolving_ns.insert(name.clone());

            // Ensure we remove the name when we're done (using a guard pattern)
...
mod tests {
    ...
    fn test_root_hints() {
        ...
        assert!(
            servers
                .iter()
                .any(|s| s.ip() == IpAddr::V4(Ipv4Addr::new(198, 41, 0, 4)))
        );
    }
